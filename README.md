...
| **DepartmentID**        | 整数（序列码）      | 外键（FK）                       | 员工所属部门。                                       |  
...
| **属性名称**          | **数据类型**         | **约束**                         | **说明**                                                                 |  
|-----------------------|----------------------|----------------------------------|-------------------------------------------------------------------------|  
| **SiteID**            | 字符串（5字符）       | 主键（PK）                       | 站点唯一代码（如ORL、DEN、MAR），字母编码设计                               |  
| **SiteName**          | 字符串（50字符）      | 非空（NOT NULL）                 | 站点全名（如Orlando, Denver）                                            |  
| **Address**           | 字符串（200字符）     | 非空（NOT NULL）                 | 完整地址（街道、城市、州、邮编）                                           |  
| **EmployeeCount**     | 整数                 | 默认值0                         | 当前员工数量（动态计算，非冗余存储）                                       |  

---

### **Department（部门）实体全属性表**  

| **属性名称**          | **数据类型**         | **约束**                         | **说明**                                                                 |  
|-----------------------|----------------------|----------------------------------|-------------------------------------------------------------------------|  
| **DepartmentID**      | 整数（序列码）        | 主键（PK）                       | 部门唯一标识符                                                          |  
| **DepartmentName**    | 字符串（50字符）      | 非空（NOT NULL）                 | 部门全名（如"Human Resources", "Engineering"）                          |  
| **SiteID**            | 字符串（5字符）       | 外键（FK，引用Site表）            | 所属站点代码                                                            |
---
